...
# Each prolongation extends the GateSeal by exactly one year (prolongation period).
...
# and no more than two prolongation periods.
...
# Prolongation window begins DAO_RESERVE + window length before expiry
...
def __init__(
    ...
    # no limits on the seal duration are enforced
...
def seal():
    ...
    assert msg.sender == SEALING_COMMITTEE, "sender: not SEALING_COMMITTEE"
    assert not self._is_expired(), "gate seal: expired"
    _sealables: DynArray[address, MAX_SEALABLES] = self.sealables
